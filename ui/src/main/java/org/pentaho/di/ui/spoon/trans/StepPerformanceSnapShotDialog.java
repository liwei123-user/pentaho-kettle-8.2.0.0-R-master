...
public class StepPerformanceSnapShotDialog extends Dialog {
	...
	public void open() {
		...
		dataList.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				// If there are multiple selections here AND there are multiple selections in
				// the steps list, we only take the
		...
		stepsList.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				// If there are multiple selections here AND there are multiple selections in
				// the data list, we only take the
	...
	private void updateCanvas() {
		 ...
		 * title BaseMessages.getString( PKG,
		 * "StepPerformanceSnapShotDialog.TimeInSeconds.Label", Integer .toString(
		 * totalTimeInSeconds ), Long.toString( timeDifference ) ), // domain axis label
		 * BaseMessages.getString( PKG,
		 * "StepPerformanceSnapShotDialog.RowsPerSecond.Label" ), // range axis label
		 * dataset, // data PlotOrientation.VERTICAL, // orientation true, // include
		 * legend true, // tooltips false ); // urls chart.setBackgroundPaint(
		 * Color.white );
		 */
		/*
		 * CategoryPlot plot = (CategoryPlot) chart.getPlot(); plot.setBackgroundPaint(
		 * Color.white ); plot.setForegroundAlpha( 0.5f );
		 ...
		 * renderer.setBaseShapesVisible( true ); renderer.setDrawOutlines( true );
		 * renderer.setUseFillPaint( true ); renderer.setBaseFillPaint( Color.white );
		...
		/*
		 * BufferedImage bufferedImage = chart.createBufferedImage( bounds.width,
		 * bounds.height ); ImageData imageData = ImageUtil.convertToSWT( bufferedImage
		 * );
		 */
